...
export async function createDevice(){const token=b64(crypto.getRandomValues(new Uint8Array(32)));const id=await hash(token);const settings={...defaults};try{Object.assign(settings,await defaultLocation());}catch{/* UI exposes a retryable location error. */}await db().prepare('INSERT INTO devices(id,settings,revision,updated) VALUES(?,?,0,?)').bind(id,JSON.stringify(settings),Date.now()).run();return {token,device:(await db().prepare('SELECT * FROM devices WHERE id=?').bind(id).first<Device>())!};}
...
export async function ensureDistrict(d:Device){const s=parseSettings(d);if(s.districtId)return d;Object.assign(s,await defaultLocation());await db().prepare('UPDATE devices SET settings=? WHERE id=? AND revision=?').bind(JSON.stringify(s),d.id,d.revision).run();return (await db().prepare('SELECT * FROM devices WHERE id=?').bind(d.id).first<Device>())!;}
...
export async function state(d:Device){
 await finishDue(d.id);const districtWasMissing=!parseSettings(d).districtId;let days:any[]=[];let prayerError='';try{d=await ensureDistrict(d);days=await prayerDays(parseSettings(d).districtId);if(!days.some(x=>x.date===dateTR()))throw Error('Bugünün vakitleri henüz alınamadı.');if(districtWasMissing&&d.subscription){await schedule(d);await restoreActive(d);}}catch(e){prayerError=e instanceof Error?e.message:'Vakitlere ulaşılamıyor.';}
